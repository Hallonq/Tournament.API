...
public class TournamentService(IUnitOfWork unitOfWork, IMapper mapper) : ITournamentService
{
    ...
    public async Task<IEnumerable<TournamentDto>> GetAllTournamentsAsync(bool includeGames, PaginationParameters paginationParameters)
    {
        var entities = await unitOfWork.TournamentRepository.GetAllAsync(includeGames, paginationParameters);
